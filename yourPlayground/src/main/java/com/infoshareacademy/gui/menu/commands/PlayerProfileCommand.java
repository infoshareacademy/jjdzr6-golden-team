...
@Component
@Order(4)
class PlayerProfileCommand implements ConsoleCommand {
    ...
    PlayerProfileCommand (ApplicationContext context) {
        this.input = requireNonNull("4").toLowerCase(Locale.ROOT);
    ...
    public void handle(String input) throws IOException {
        if(continueHandler.continueHandler()) {
            System.out.println("Player profile soon!");
        }
    }

    @Override
    public String introduce() {
        return input + ". Player profile";
    }
}
